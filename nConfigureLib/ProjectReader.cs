...
namespace nConfigureLib
{
	...
	{
        static Logger log = new Logger(typeof(Project));
		public static Project ReadProject(string fullAbsoluteFileName)
		{
			
            try
			{
                var project = new Project(fullAbsoluteFileName);
		...
		{
            log.Debug(string.Format("Converting to absolut path: {0} {1}", csProjectPath, path));
